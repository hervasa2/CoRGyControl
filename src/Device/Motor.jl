mutable struct Motor <: Device
    name::String
    id::Int
    axis_length::Value
    motor_position::Value
    encoder_position::Value
    unit::Union{typeof(u"mm"), typeof(u"°")}
    calibrated::Bool
    safe_to_move::Bool

    Motor(name::String, id::Int, unit::Union{typeof(u"mm"), typeof(u"°")}, safe_to_move::Bool) = new(name, id, missing, missing, missing, unit, false, safe_to_move)
...
function convert_steps_to_unit(m::Motor, steps::Union{Int16, Int32, Int64, Missing})::Value
    ...
    elseif m.unit == u"°"
        ...
        error("$(m.name) hat not the right unit. Must be 'mm' or '°'")
    end
end
...
function update_axis_length!(m::Motor, motor_controller::TMCM3110)::Nothing
    try
        steps = -1*get_axis_parameter(motor_controller, 196, m.id  )
...
function calibrate_motors(ms::Array{Motor}, motor_controller::TMCM3110)::Nothing
    stm = [ m.safe_to_move for m in ms ]
    if !(sum(stm) == length(stm))
        @warn "Motors are not safe to be moved. Skipping Calibration."
        return nothing
    else
        start_calibration = 0
        get_status = 2
